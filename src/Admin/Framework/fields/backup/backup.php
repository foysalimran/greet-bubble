...
if ( ! class_exists( 'GREET_BUBBLE_Field_backup' ) ) {
  class GREET_BUBBLE_Field_backup extends GREET_BUBBLE_Fields {
    ...
    public function render() {
      ...
      echo '<textarea name="greet_bubble_import_data" class="greet-bubble-import-data"></textarea>';
      echo '<button type="submit" class="button button-primary greet-bubble-confirm greet-bubble-import" data-unique="'. esc_attr( $unique ) .'" data-nonce="'. esc_attr( $nonce ) .'">'. esc_html__( 'Import', 'greet-bubble' ) .'</button>';
      echo '<hr />';
