 ...
 *
 */
if ( ! class_exists( 'GREET_BUBBLE_Field_dimensions' ) ) {
  class GREET_BUBBLE_Field_dimensions extends GREET_BUBBLE_Fields {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' greet-bubble--is-unit' : '';
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="greet-bubble--label greet-bubble--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="greet-bubble-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="greet-bubble--label greet-bubble--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="greet-bubble--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="greet-bubble--label greet-bubble--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="greet-bubble-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="greet-bubble--label greet-bubble--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
