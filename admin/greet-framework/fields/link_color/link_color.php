 ...
 *
 */
if ( ! class_exists( 'GREET_Field_link_color' ) ) {
  class GREET_Field_link_color extends GREET_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'greet' ),
        'hover'    => esc_html__( 'Hover', 'greet' ),
        'active'   => esc_html__( 'Active', 'greet' ),
        'visited'  => esc_html__( 'Visited', 'greet' ),
        'focus'    => esc_html__( 'Focus', 'greet' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="greet--left greet-field-color">';
          echo '<div class="greet--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="greet-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
